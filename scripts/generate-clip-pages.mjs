...
async function main() {
  ...
  // ---- Monthly archive pages + OG images + sub-feeds ----
  const monthSummaries = await generateMonthlyArchives({ clips, template });

  // ---- Sitemap for search engines ----
  const sitemap = generateSitemap(clips, { tagSummaries, monthSummaries });
  await writeFile(path.join(DIST, 'sitemap.xml'), sitemap);
  console.log(
    `✓ Sitemap written (dist/sitemap.xml, ${
      clips.length + tagSummaries.length + monthSummaries.length + 1
    } URLs).`,
  );
...
async function generateMonthlyArchives({ clips, template }) {
  const buckets = bucketByMonth(clips);
  if (buckets.size === 0) {
    console.log('! No dated clips found; skipping monthly archives.');
    return [];
  }

  console.log(
    `→ Pre-rendering ${buckets.size} monthly archive page${buckets.size === 1 ? '' : 's'}…`,
  );
  const summaries = [];

  for (const [yearMonth, monthClips] of buckets) {
    const sortedClips = [...monthClips].sort((a, b) => {
      const aT = new Date(a.addedAt || a.observedDate).getTime() || 0;
      const bT = new Date(b.addedAt || b.observedDate).getTime() || 0;
      return bT - aT;
    });
    const [year, month] = yearMonth.split('-');
    const outDir = path.join(DIST, 'clips', year, month);
    await mkdir(outDir, { recursive: true });

    const canonicalUrl = `${SITE_BASE}/clips/${year}/${month}/`;
    const ogImageUrl = `${SITE_BASE}/clips/${year}/${month}/og.png`;
    const feedUrl = `${SITE_BASE}/clips/${year}/${month}/feed.xml`;
    const displayName = `${MONTH_NAMES[Number(month) - 1]} ${year}`;

    const svg = generateMonthSvg(yearMonth, displayName, sortedClips);
    const png = await renderPng(svg);
    await writeFile(path.join(outDir, 'og.png'), png);

    const html = injectMonthMeta(template, yearMonth, displayName, sortedClips, {
      canonicalUrl,
      ogImageUrl,
      feedUrl,
    });
    ...
    const subFeed = generateAtomFeed(sortedClips, {
      title: `Economic Dashboard — ${displayName}`,
      subtitle: `Clips added in ${displayName}, by Golden Data.`,
      selfUrl: feedUrl,
      altUrl: canonicalUrl,
    });
    await writeFile(path.join(outDir, 'feed.xml'), subFeed);
    ...
    console.log(
      `  ✓ ${BASE_PATH}/clips/${year}/${month}/  (${sortedClips.length} clip${sortedClips.length === 1 ? '' : 's'}, og ${png.length}B)`,
    );
  }
  return summaries;
}
...
function generateSitemap(clips, { tagSummaries = [], monthSummaries = [] } = {}) {
  const urls = [
    {
      loc: `${SITE_BASE}/`,
...
// ---- Monthly archive OG image renderer ----
function generateMonthSvg(yearMonth, displayName, clips) {
  const W = 1200;
  const H = 630;
  const PAD = 56;
  const count = clips.length;
  ...
  const pill = `
    <rect x="${PAD}" y="${PAD - 6}" width="86" height="26" rx="13" ry="13"
          fill="${COLOR.brandAccent}22" stroke="${COLOR.brandAccent}" stroke-width="1"/>
    <text x="${PAD + 43}" y="${PAD + 12}" text-anchor="middle"
          font-family="Inter, system-ui, sans-serif" font-size="13" font-weight="700"
          letter-spacing="2" fill="${COLOR.brandAccent}">ARCHIVE</text>
  `;
  ...
  const titleEl = `
    <text x="${PAD}" y="${titleY}" font-family="Inter, system-ui, sans-serif"
          font-size="92" font-weight="800" letter-spacing="-2"
          fill="${COLOR.brandPrimary}">${escapeXml(displayName)}</text>
  `;

  const subtitleEl = `
    <text x="${PAD}" y="${subtitleY}" font-family="Inter, system-ui, sans-serif"
          font-size="22" font-weight="600" fill="${COLOR.textMuted}">
      ${count} clip${count === 1 ? '' : 's'} collected this month · Golden Data
    </text>
  `;
  ...
  const moreText =
    count > visibleClips.length ? `+ ${count - visibleClips.length} more` : '';
  const moreEl = moreText
    ? `<text x="${PAD}" y="${listTop + visibleClips.length * lineH + 14}"
            font-family="Inter, system-ui, sans-serif" font-size="14"
            fill="${COLOR.textMuted}">${escapeXml(moreText)}</text>`
  ...
  const footer = `
    ...
    <text x="${W - PAD}" y="${footerY + 6}" text-anchor="end"
          font-family="Inter, system-ui, sans-serif" font-size="14"
          fill="${COLOR.textMuted}">browningtons.github.io/economic-dashboard/clips/${escapeXml(yearMonth.replace('-', '/'))}/</text>
  `;
...
function injectMonthMeta(template, yearMonth, displayName, clips, { canonicalUrl, ogImageUrl, feedUrl }) {
  const count = clips.length;
  const title = `${displayName} — Economic Dashboard Clips`;
  const description = `${count} clip${count === 1 ? '' : 's'} added in ${displayName} on the Economic Dashboard, by Golden Data.`;
  ...
  // SPA hint: which month should be active on load
  const monthHint = `<meta name="active-month" content="${escapeHtmlAttr(yearMonth)}" />`;
  if (/<meta\s+name="active-month"/i.test(out)) {
    out = out.replace(/<meta\s+name="active-month"[^>]*>/i, monthHint);
  } else {
    out = out.replace(/<\/head>/i, `    ${monthHint}\n  </head>`);
  }

  // Atom sub-feed auto-discovery
  const altFeedTag = `<link rel="alternate" type="application/atom+xml" title="${escapeHtmlAttr(displayName)} — Economic Dashboard" href="${escapeHtmlAttr(feedUrl)}" />`;
  const altFeedRe = /<link\s+rel="alternate"\s+type="application\/atom\+xml"[^>]*>/i;
  if (altFeedRe.test(out)) out = out.replace(altFeedRe, altFeedTag);
  else out = out.replace(/<\/head>/i, `    ${altFeedTag}\n  </head>`);
