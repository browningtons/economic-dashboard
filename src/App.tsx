...
export default function App() {
  ...
  const [csvSource, setCsvSource] = useState<CsvSource>('local');
  ...
  const [activeTab, setActiveTab] = useState<'Dashboard' | 'Buffett' | 'Data Table' | 'Clips'>(() => {
    ...
    return clipHint || isClipsPath ? 'Clips' : 'Dashboard';
  });
  ...
  useEffect(() => {
    ...
    const getCsvData = async (): Promise<{ text: string; source: CsvSource }> => {
      ...
      if (USE_GOOGLE_SHEET && GOOGLE_SHEET_URL) {
        ...
        try {
          ...
          if (response.ok) return { text: await response.text(), source: 'sheet' };
      ...
      try {
        ...
        console.warn(`Failed to fetch local CSV (status ${response.status}). Falling back to embedded CSV.`);
      } catch (error) {
    ...
    const loadData = async () => {
      ...
      try {
        const { text: csvData, source: csvDataSource } = await getCsvData();
        if (cancelled) return;
        ...
        // Two independent warnings can be true at once: the rows came from the
        // bundled snapshot, and/or some rows were malformed. Show both.
        const latestPointLabel = new Date(
          parsedData[parsedData.length - 1].timestamp,
        ).toLocaleDateString(undefined, { month: 'long', year: 'numeric' });
        const warnings = [
          fallbackWarning(csvDataSource, latestPointLabel),
          badRowCount > 0
            ? `Loaded with ${badRowCount} skipped row(s) due to malformed CSV rows.`
            : null,
        ...
        if (!cancelled) {
          ...
          if (!pipelineFreshnessAppliesTo(csvDataSource)) {
            notify.warning(`Live data unavailable — showing the bundled snapshot through ${latestPointLabel}.`);
          } else if (badRowCount > 0) {
          ...
          // The status file describes the pipeline's last run, which never
          // touched the bundled snapshot — don't report on its behalf. The
          // fallback warning above already covers that case, and a PASS here
          // would contradict it.
          if (pipelineFreshnessAppliesTo(csvDataSource)) {
            if (!status) {
  ...
  const lastUpdatedText = useMemo(() => {
    // Only date the rows on screen with the pipeline's run time when the
    // pipeline actually produced them. On the embedded fallback the status
    // file can be fresh and PASS while the chart is the bundled snapshot —
    // reporting `generatedAt` there would put today's timestamp on old data.
    if (pipelineFreshnessAppliesTo(csvSource) && pipelineStatus?.generatedAt) {
