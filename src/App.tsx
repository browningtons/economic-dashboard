...
export default function App() {
  ...
  const sourceLabel = dataSource === 'google' ? 'Google Sheet (opt-in)' : dataSource === 'local' ? 'Auto-updated CSV' : 'Embedded fallback';
  ...
  const monthsBehind = useMemo(() => {
    ...
    return ((current.getFullYear() - latest.getFullYear()) * 12) + (current.getMonth() - latest.getMonth());
  }, [clockNow, data]);
  const refreshUrgency = useMemo(() => {
    if (monthsBehind >= 2) return { label: 'High refresh urgency', className: 'text-[color:var(--color-brand-primary)]' };
    if (monthsBehind === 1) return { label: 'Medium refresh urgency', className: 'text-link' };
    return { label: 'Low refresh urgency', className: 'text-muted' };
  }, [monthsBehind]);
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 bg-secondary p-7 rounded-2xl shadow-sm border border-theme/70">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex basis-full flex-col gap-1 text-[11px] leading-tight text-muted md:basis-auto md:items-end md:ml-2">
            ...
            <span className={refreshUrgency.className}>{refreshUrgency.label}</span>
            {refreshStatus && (
